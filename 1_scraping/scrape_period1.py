...
def scrape_exam(date_str):
    ...
    url = f"https://cbtbank.kr/exam/cek{date_code}"  # Changed from cem to cek
...
if __name__ == '__main__':
    ...
    if os.path.exists(db_path):
        ...
        # Add period info to older questions if not present
        for q in all_questions:
            if 'period' not in q:
                q['period'] = '2' # Previously scraped ones were 2교시

    print(f"Loaded {len(all_questions)} existing questions.")
    
    initial_count = len(all_questions)
    for d in dates:
        print(f"Scraping exam 1교시 for {d}...")
    ...
    print(f"Successfully added {len(all_questions) - initial_count} questions. Total: {len(all_questions)}")
