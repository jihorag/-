...
dates_cgi = ["20250524", "20240525"]
dates_cfr = ["20230527", "20220514", "20190601", "20170520", "20150606", "20140607"]
...
def scrape_exam(prefix, date):
    url = f"{base_url}/exam/{prefix}{date}"
...
def main():
    ...
    new_added = 0
    all_dates = [(d, "cgi") for d in dates_cgi] + [(d, "cfr") for d in dates_cfr]
    
    for date, prefix in all_dates:
        date_str = f"{date[:4]}-{date[4:6]}-{date[6:]}"
        if any(q.get('period') == prefix and q.get('exam_date') == date_str for q in db):
            print(f"Skipping {date_str} (exists)")
            continue
            
        print(f"Scraping {date_str} ({prefix})...")
        qs = scrape_exam(prefix, date)
