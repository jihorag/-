...
def main():
    ...
    tasks = [
        {
            "pdf": "sources/기출문제/세무사/2021/2021년도 제58회 세무사 1차 시험 1교시 A형.pdf",
            "year": "2021", "session": 1, "subjects": [("재정학", 1, 40)]
        },
        {
            "pdf": "sources/기출문제/세무사/2021/2021년도 제58회 세무사 1차 시험 2교시 A형(민법).pdf",
    ...
    for task in tasks:
        ...
        for subject, min_q, max_q in task["subjects"]:
            ...
            # 민법 등은 후반부 페이지에 있음 (최적화)
            start_p = 0
            if subject == "민법" or subject == "세법학개론":
                start_p = 15
                
            for page_num in range(start_p, len(doc)):
                page = doc[page_num]
