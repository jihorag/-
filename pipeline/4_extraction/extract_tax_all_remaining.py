...
def main():
    ...
    pdf_files = sorted(glob.glob("sources/기출문제/세무사/*/*.pdf"))
    output_dir = "images"
    ...
    for pdf_path in pdf_files:
        ...
        session = 1 if "1교시" in normalize_nfc(filename) else 2
        
        # 건너뛸 이미 처리된 파일들 조건 (필요시 조정 가능)
        # 2024년 2교시 민법은 별도 스크립트에서 진행 중이므로 패스
        if year == 2024 and session == 2 and "민법" in normalize_nfc(filename):
