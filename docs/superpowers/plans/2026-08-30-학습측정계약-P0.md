...
- **`useRef(Date.now())` 를 쓰지 않는다.** 이 저장소의 eslint 는 `react-hooks/purity` 를 에러로 잡는다(렌더 중 불순 함수 호출). 소요시간 기준점은 `useRef(0)` 로 두고 `useEffect` 에서 채우며, 읽을 때 `shownAtRef.current ? ... : undefined` 로 방어한다.
- 각 태스크는 자기가 만진 파일에 대해 `npx eslint <파일>` 이 **새 에러 0** 인지 확인한다. 저장소 전체 린트는 기존 경고가 많아 기준이 되지 못한다.
...
```js
  ...
  // useRef(Date.now()) 로 쓰면 react-hooks/purity 린트 에러다 — 렌더 중 불순 함수 호출.
...
`ParsedText.jsx` 상단 React import 에 `useRef`·`useEffect` 가 없으면 추가한다.
...
```js
  const checkTable = () => {
    ...
    recordItem({ kind: 'prac', idx: seq, q: body('문제') || caption, isCorrect: ok, gradedBy: 'machine', ms: shownAtRef.current ? Date.now() - shownAtRef.current : undefined });
  };
  const grade = (ok) => {
    setGraded(ok);
    // 사용자가 O/X 를 눌렀다 — 자기 채점이다. 무게가 기계 채점의 1/6 이다.
    recordItem({ kind: 'prac', idx: seq, q: body('문제'), isCorrect: ok, gradedBy: 'self', ms: shownAtRef.current ? Date.now() - shownAtRef.current : undefined });
  };
```
