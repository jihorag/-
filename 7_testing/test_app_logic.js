const fs = require('fs');
const questionsData = JSON.parse(fs.readFileSync('questions_db.json', 'utf-8'));
...
const taxData = processedData.filter(q => q.exam === '세무사');
const subjects = new Set(taxData.map(q => q.subject));
const unifiedSubjects = new Set(taxData.map(q => q.unifiedSubject));
console.log('Subjects for 세무사:', Array.from(subjects));
console.log('Unified for 세무사:', Array.from(unifiedSubjects));
