...
# We are verifying questions 1101 to 1150 (0-indexed 1100 to 1149)
target_range = range(1100, 1150)
target_questions = [questions[i] for i in target_range if i < len(questions)]

print(f"Loaded {len(target_questions)} questions from target range (1101~1150).")
...
for idx, q in enumerate(target_questions):
    ...
    # 1. ID check
    q_id = q.get("id", "")
    if not q_id.startswith("practice-accounting-ch04s04-"):
    ...
    if len(options) != 5:
        ...
        # Check circular numbers ① to ⑤
        circs = ["①", "②", "③", "④", "⑤"]
        for opt_idx, opt in enumerate(options):
            if not opt.strip().startswith(circs[opt_idx]):
                errors.append(f"Q{q_num}: Option {opt_idx+1} does not start with '{circs[opt_idx]}'")
