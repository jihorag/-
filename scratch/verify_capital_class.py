...
# Verify range 2501 to 2550 (index 2500 to 2549)
target_range = range(2500, 2550)
target_questions = []
for i in target_range:
    if i < len(questions):
        target_questions.append(questions[i])

print(f"Loaded {len(target_questions)} questions from target range (2501~2550).")
...
for idx, q in enumerate(target_questions):
    ...
    # 1. ID check
    q_id = q.get("id", "")
    if not q_id.startswith("practice-accounting-ch10s02-"):
    ...
    if len(options) != 5:
        ...
        for opt_idx, opt in enumerate(options):
            # Strip markdown bold marks like ** ① ... ** or similar, or just check standard starts
            cleaned_opt = opt.strip().lstrip("•").strip().lstrip("*").strip().lstrip("-").strip()
            if not cleaned_opt.startswith(circs[opt_idx]):
