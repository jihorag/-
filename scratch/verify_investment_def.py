...
# Verify range 1401 to 1450 (index 1400 to 1449)
target_range = range(1400, 1450)
target_questions = []
for i in target_range:
    if i < len(questions):
        target_questions.append(questions[i])

print(f"Loaded {len(target_questions)} questions from target range (1401~1450).")
...
for idx, q in enumerate(target_questions):
    ...
    # 1. ID check
    q_id = q.get("id", "")
    if not q_id.startswith("practice-accounting-ch06s01-"):
