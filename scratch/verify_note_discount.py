...
# Verify range 1951 to 2000 (index 1950 to 1999)
target_range = range(1950, 2000)
target_questions = []
for i in target_range:
    if i < len(questions):
        target_questions.append(questions[i])

print(f"Loaded {len(target_questions)} questions from target range (1951~2000).")
...
for idx, q in enumerate(target_questions):
    ...
    # 1. ID check
    q_id = q.get("id", "")
    if not q_id.startswith("practice-accounting-ch07s08-"):
