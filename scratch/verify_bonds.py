...
# Verify range 2051 to 2100 (index 2050 to 2099)
target_range = range(2050, 2100)
target_questions = []
for i in target_range:
    if i < len(questions):
        target_questions.append(questions[i])

print(f"Loaded {len(target_questions)} questions from target range (2051~2100).")
...
for idx, q in enumerate(target_questions):
    ...
    # 1. ID check
    q_id = q.get("id", "")
    if not q_id.startswith("practice-accounting-ch08s01-"): # Wait, let's see. In taxonomy, Chapter 08 Section 2 might map to s02 or s01 depending on sections.
        # Wait, let's check s02 or s01.
        # Chapter 08 sections in taxonomy are:
        # Chapter 08 금융부채: 1절 금융부채의 인식과 측정, 2절 사채, 3절 사채의 후속측정, 4절 금융부채로 분류하는 상환우선주
        # Oh, in implementation plan, we said "Ch08 2절 사채" which is the 2nd item of Chapter 08.
        # So we should use "practice-accounting-ch08s02-" as ID prefix.
        pass
    
    if not q_id.startswith("practice-accounting-ch08s02-"):
