...
# Verify range 1201 to 1250 (index 1200 to 1249)
target_range = range(1200, 1250)
target_questions = [questions[i] for i in target_range if i < len(questions)]

print(f"Loaded {len(target_questions)} questions from target range (1201~1250).")
...
for idx, q in enumerate(target_questions):
    ...
    # 1. ID check
    q_id = q.get("id", "")
    if not q_id.startswith("practice-accounting-ch04s06-"):
