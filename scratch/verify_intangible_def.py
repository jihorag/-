...
# Verify range 1251 to 1300 (index 1250 to 1299)
target_range = range(1250, 1300)
target_questions = [questions[i] for i in target_range if i < len(questions)]

print(f"Loaded {len(target_questions)} questions from target range (1251~1300).")
...
for idx, q in enumerate(target_questions):
    ...
    # 1. ID check
    q_id = q.get("id", "")
    if not q_id.startswith("practice-accounting-ch05s01-"):
