...
# Verify range 1751 to 1800 (index 1750 to 1799)
target_range = range(1750, 1800)
target_questions = []
for i in target_range:
    if i < len(questions):
        target_questions.append(questions[i])

print(f"Loaded {len(target_questions)} questions from target range (1751~1800).")
...
for idx, q in enumerate(target_questions):
    ...
    # 1. ID check
    q_id = q.get("id", "")
    if not q_id.startswith("practice-accounting-ch07s04-"):
