...
# Verify range 2451 to 2500 (index 2450 to 2499)
target_range = range(2450, 2500)
target_questions = []
for i in target_range:
    if i < len(questions):
        target_questions.append(questions[i])

print(f"Loaded {len(target_questions)} questions from target range (2451~2500).")
...
for idx, q in enumerate(target_questions):
    ...
    # 1. ID check
    q_id = q.get("id", "")
    if not q_id.startswith("practice-accounting-ch10s01-"):
    ...
    if q.get("question_type") not in ["개념5지", "이론형", "판례형"]:
        # Wait, let's keep question_type matching our generator which is "개념5지"
