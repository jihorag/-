...
# Verify range 1301 to 1350 (index 1300 to 1349)
target_range = range(1300, 1350)
target_questions = [questions[i] for i in target_range if i < len(questions)]

print(f"Loaded {len(target_questions)} questions from target range (1301~1350).")
...
for idx, q in enumerate(target_questions):
    ...
    # 1. ID check
    q_id = q.get("id", "")
    if not q_id.startswith("practice-accounting-ch05s02-"):
