...
[⭐ 시각자료 — viz 템플릿 우선]
- **그래프·도식·관계도가 필요하면 반드시 \`\`\`viz <template-name> 펜스 사용**.
- 사용 가능한 템플릿은 인수인계서의 [VIZ_CATALOG] 섹션 참조 — 거기 명시된 것만 사용.
- **freeform SVG 절대 금지** (\`<svg>...\` 직접 작성 X). 카탈로그에 없으면 텍스트/표로 설명.
- viz 펜스 안엔 **JSON만** 작성. 주석·trailing comma 금지 (표준 JSON).
- 차트는 의미적 파라미터만 채움. 좌표·색·축 라벨은 컴포넌트가 처리.
- 차트 뒤엔 본문 해설을 마크다운으로 이어 작성 — 차트만 단독으로 던지지 말 것.
- 한 메시지에 viz 1개만 (학습 부담 줄이기).
