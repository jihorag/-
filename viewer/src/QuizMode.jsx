...
function DrillSession({ subjectId, leaf, topic, onBack, onProgress, nextTopic, onNext, onGoSolve }) {
  ...
  const system = `너는 감정평가사 1차 수험 '드릴 코치'다. 이번 드릴에서 다루는 좁은 개념은 오직 "${topic.title}" 하나다.
...
[피드백 — 짧고 정직하게] 맞으면 한 줄 인정 + 빠진 키워드 보강. 틀리거나 빠지면 "무엇이 빠졌는지"를 1~2문장으로 정확히 짚어라(칭찬만 X). 가능하면 떠올리기 쉬운 트리거 키워드 2~3개나 두문자로 묶어줘라("이 단어만 보면 ~가 떠오르게").
