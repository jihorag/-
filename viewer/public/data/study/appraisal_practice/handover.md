...
{"scenario": "<한 줄>",
 ...
 "base_note": "정상거래",
 "adjustments": [
   {"factor": "사정보정", "rate": 0, "reason": "정상거래"},
   {"factor": "시점수정", "rate": 0.03, "reason": "지가지수 +3%"},
...
**사용 시점**: 비교방식 거래사례비교법 답안 흐름 / 사정·시점·지역·개별 보정 단계 / 보정률 함정 (음수·복합 보정) 학습 시.  
**규칙**: 적용 우선순위 (사정 → 시점 → 지역 → 개별) 순으로 배열. rate 는 비율 (0.05 = +5%). 컴포넌트가 단계별 누적 계산 + 총 보정률 자동.

## 차트 공통 규칙
- 한 메시지에 viz 1개만.
- 차트는 답안 흐름 설계 + 산식 검증용. 본문 답안은 텍스트로 작성.
- 카탈로그(income-capitalization·dcf-timeline·sales-adjustment)가 평가 3대 방식 중 수익+비교를 커버. 원가방식·임대료 산정은 Mermaid 또는 텍스트로.
