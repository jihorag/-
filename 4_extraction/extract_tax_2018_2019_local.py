...
OCR 과정에서 발생한 오타(예: '쑤' -> '※', '설명브토' -> '설명으로' 등)와 띄어쓰기 오류를 **문맥에 맞게 자연스러운 한국어로 반드시 교정**하여 ①부터 ⑤까지의 보기를 정제해 주세요.
...
def main():
    ...
    tasks = [
        # 2019
        {"pdf": "기출문제/세무사/2019/1교시 A형.pdf", "year": "2019", "session": 1, "subjects": [("재정학", 1, 40)]},
        {"pdf": "기출문제/세무사/2019/2교시 B형(회계학개론, 민법).pdf", "year": "2019", "session": 2, "subjects": [("회계학개론", 1, 40), ("민법", 41, 80)]},
        # 2018
        {"pdf": "기출문제/세무사/2018/2018 세무사 1차 1교시 A형.pdf", "year": "2018", "session": 1, "subjects": [("재정학", 1, 40)]},
        {"pdf": "기출문제/세무사/2018/2018 세무사 1차 2교시 A형(민법).pdf", "year": "2018", "session": 2, "subjects": [("회계학개론", 1, 40), ("민법", 41, 80)]},
    ]
    ...
    for task in tasks:
        ...
        print(f"\n>> Processing {year} {os.path.basename(pdf_path)}...")
        if not os.path.exists(pdf_path): continue
            
        doc = fitz.open(pdf_path)
        
        for subject, min_q, max_q in task["subjects"]:
            print(f"  == Subject: {subject} (Q{min_q}~{max_q}) ==")
            ...
            for page_num in range(len(doc)):
                ...
                for w in words:
                    ...
                    if re.match(r'^\d+\.?$', text):
                        try:
                            ...
                            if min_q <= q_num <= max_q and w[0] < 150:
                ...
                for i, q_info in enumerate(q_starts):
                    ...
                    # Crop body (up to above option 1)
                    y_top = max(0, q_y0 - 5)
                    ...
                    q_data = {
                        ...
                        "tags": llm_res.get("tags", {"subject": subject, "difficulty": 3})
                    }
