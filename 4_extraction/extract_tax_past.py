...
def main():
    ...
    tasks = [
        {
            "pdf": "기출문제/세무사/2024/2024년도 제61회 세무사 1차시험 2교시 시험지 원본(민법).pdf",
            "year": "2024", "session": 2, "subjects": [("민법", 41, 80)]
        },
        {
            "pdf": "기출문제/세무사/2023/2023년 제60회 세무사 1차시험 1교시 문제지.pdf",
            "year": "2023", "session": 1, "subjects": [("재정학", 1, 40)]
        },
        {
            "pdf": "기출문제/세무사/2023/2023년 제60회 세무사 1차시험 2교시 문제지(민법).pdf",
            "year": "2023", "session": 2, "subjects": [("회계학개론", 1, 40), ("민법", 41, 80)]
        },
        {
            "pdf": "기출문제/세무사/2022/2022년도 제59회 세무사 1차 시험 2교시 문제지 A형(민법).pdf",
            "year": "2022", "session": 2, "subjects": [("회계학개론", 1, 40), ("민법", 41, 80)]
        }
    ]
    ...
    for task in tasks:
        ...
        for subject, min_q, max_q in task["subjects"]:
            ...
            # 민법은 보통 후반부 페이지에 있음 (최적화)
            start_page = 0
            if subject == "민법" or subject == "세법학개론":
                start_page = 15 # 절반 이상 건너뛰기
                
            for page_num in range(start_page, len(doc)):
                # 이미 해당 과목의 문제가 모두 추출되었는지 확인
                existing_q_nums = [
                    int(q['number']) for q in main_db 
                    if q['year'] == year and q['subject'] == subject and min_q <= int(q.get('number', 0)) <= max_q
                ]
                if len(set(existing_q_nums)) == (max_q - min_q + 1):
                    print(f"    [Skip] {subject} all questions ({min_q}~{max_q}) are already extracted.")
                    break
