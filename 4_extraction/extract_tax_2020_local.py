...
def main():
    ...
    tasks = [
        {
            "pdf": "기출문제/세무사/2020/1교시 A형.pdf",
            "year": "2020", "session": 1, "subjects": [("재정학", 1, 40)]
        },
        {
            "pdf": "기출문제/세무사/2020/2교시 A형(선택과목 민법).pdf",
            "year": "2020", "session": 2, "subjects": [("회계학개론", 1, 40), ("민법", 41, 80)]
        }
    ]
            ...
            for page_num in range(len(doc)):
                ...
                for w in words:
                    ...
                    if re.match(r'^\d+\.$', text):
                        try:
                            q_num = int(text[:-1])
                            if min_q <= q_num <= max_q and w[0] < 120:
